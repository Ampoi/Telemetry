...
export function commonSlot(p: PollState, now: number): number | undefined {
  ...
  for (const day of [2, 1, 3, 0, 4]) for (let slot = 0; slot <= 48 - p.duration / 30; slot++) {
    const index = day * 48 + slot;
    if (p.start + index * SLOT <= now + 3600_000) continue;
    if (sets.every(s => Array.from({ length: p.duration / 30 }, (_, n) => index + n).every(n => s.has(n)))) return p.start + index * SLOT;
  }
}
